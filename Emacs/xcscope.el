...
;
; File:         xcscope.el
; RCS:          $RCSfile: xcscope.el,v $ $Revision: 1.9 $ $Date: 2001/06/28 22:15:00 $ $Author: darrylo $
...
(if cscope:map
  ...
  (define-key cscope:map "\C-csS" 'cscope-set-database-directory)
...
(easy-menu-define cscope:menu
		  ...
		  '("Cscope"
		    ...
		    [ "Set this buffer's cscope directory"
		      cscope-set-database-directory t ]
...
Starting from DIRECTORY, look upwards for a cscope database."
  (let (this-directory database-dir)
	...
	(if (or (file-exists-p (concat this-directory "build/" cscope-database-file))
		(file-exists-p (concat this-directory "build/" cscope-index-file)))
	    (progn
	      (setq database-dir (concat this-directory "build"))
	      (throw 'done database-dir)
	      ))
	(if (or (file-exists-p (concat this-directory "build/common/cscope/" cscope-database-file))
		(file-exists-p (concat this-directory "build/common/cscope/" cscope-index-file)))
	    (progn
	      (setq database-dir (concat this-directory "build/common/cscope"))
	      (throw 'done database-dir)
	      ))
...
(defun cscope-set-database-directory (top-directory)
  "Set the database directory.
Set the current database directory.  This is a hack."
  (interactive "DSet database directory: ")
  (let ()
    (cscope-unix-index-files-internal
     top-directory
     (setq cscope-database-regexps
	   (list (list (concat) (list top-directory)))
	   )
     nil)
    ))
